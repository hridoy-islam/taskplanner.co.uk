...
import { Star, CheckCircle2, RotateCcw, Eye, MessageCircle } from 'lucide-react';
...
const TaskList = ({ tasks, onMarkAsImportant, onToggleTaskCompletion }) => {
  ...
  <TableHeader className="bg-taskplanner text-white">
    <TableRow className="border-none ">
      <TableHead className="w-[40%] border border-gray-200 text-xs font-bold uppercase tracking-wider">
        Task
      </TableHead>

      <TableHead className="border border-gray-200 text-center text-xs font-bold uppercase tracking-wider">
        Priority
      </TableHead>
      ...
      <TableHead className="pr-6 border border-gray-200 text-right text-xs font-bold uppercase tracking-wider">
        Action
      </TableHead>
    </TableRow>
  </TableHeader>
  ...
  <TableBody>
    {sortedTasks.map((task) => {
      ...
      const isCompletedByAssignee =
        task.completedBy?.includes(assigneeId);

      const isImportant =
        task.importantBy?.includes(user?._id);
