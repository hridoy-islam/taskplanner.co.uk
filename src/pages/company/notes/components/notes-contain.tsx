...
export default function NotesContent({
  ...
  return (
    <div className="flex flex-1 flex-col bg-white">
      <header className="flex items-center justify-between border-b border-blue-200 p-6">
              ...
              <PopoverContent className="w-56 rounded-md border border-neutral-200 bg-white p-0 text-black shadow-lg">
                <div className="flex flex-col">
                  <input
                    ...
                    className="border-b border-neutral-100 p-3 text-sm focus:outline-none"
                  />
      ...
      className={` border-gray-200 transition-colors ${
        selectedNote.favorite 
          ? 'bg-amber-50 border-amber-200 text-amber-500 hover:text-amber-500/90 hover:bg-amber-50' 
          : 'text-gray-500 hover:text-amber-500'
      }`}
      onClick={() => handleNoteAction('favorite')}
      title="Favorite"
    >
      <Star className={`h-4 w-4 ${selectedNote.favorite ? 'fill-current' : ''}`} /> Favorite
    </Button>
      ...
      <main className="flex flex-1 flex-col overflow-hidden p-6">
        {selectedTab === 'my-notes' && selectedNote.tags?.length > 0 && (
          <div className="mb-6 flex flex-wrap gap-2">
            {selectedNote.tags.map((tag: any) => (
              <span
                key={tag?._id}
                className="flex items-center rounded-full bg-blue-500 px-3 py-1 text-sm font-medium text-white"
              >
        ...
        <div id="editor-wrapper" className="relative flex-1 ">
          <ReactQuill
            ...
            className="z-[9999] h-[70vh] [&_.ql-container]:border-none [&_.ql-editor]:text-lg [&_.ql-editor]:text-black [&_.ql-toolbar]:border-b [&_.ql-toolbar]:border-none [&_.ql-toolbar]:border-neutral-100 [&_.ql-tooltip]:!left-auto [&_.ql-tooltip]:md:!left-[0px]"
