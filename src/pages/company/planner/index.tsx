...
export default function CompanyTaskPlanner() {
  ...
  const getTaskStyles = (task: Task) => {
    ...
    const hash = task.taskName.length % 4;
    switch (hash) {
      case 0:
      ...
      default:
        return 'bg-slate-50 border-slate-200 text-slate-900 border-l-4 border-l-slate-800';
    }
  };

  return (
    <div className="flex h-screen flex-col bg-gray-50/30 p-4 md:p-6">
            ...
            return (
              ...
              >
                ...
                <div className={`sticky top-0 z-10 flex flex-col items-center justify-center border-b border-gray-100 py-3 backdrop-blur-md ${isToday ? 'bg-blue-50/90' : 'bg-white/90'}`}>
                  <span className={`text-xs font-semibold uppercase tracking-wider ${isToday ? 'text-blue-600' : 'text-gray-400'}`}>
                    {day.format('ddd')}
                  </span>
                  <span className={`mt-0.5 flex h-7 w-7 items-center justify-center rounded-full text-sm font-bold ${isToday ? 'bg-blue-600 text-white shadow-sm' : 'text-gray-800'}`}>
                    {day.format('D')}
                  </span>
                </div>
