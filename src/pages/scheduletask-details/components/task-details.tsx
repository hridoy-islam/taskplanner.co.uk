...
export default function TaskDetails({ task, onUpdate }: TaskDetailsProps) {
  ...
  const syncFormState = (taskData: any) => {
    ...
    setTempDueDate(taskData.dueDate ? new Date(taskData.dueDate) : null);
    setFrequency(taskData.frequency ?? 'once');
    setScheduledDate(taskData?.scheduledDate ?? null);
    setTempPriority(taskData.priority || 'medium');
  ...
  // Auto-set dueDate when frequency changes
  useEffect(() => {
    if (frequency === TaskFrequency.DAILY) {
      setTempDueDate(moment().toDate());
    } else if (frequency === TaskFrequency.ONCE) {
  ...
  const handleDelete = async () => {
    ...
    setIsDeleting(true);
    try {
      await axiosInstance.delete(`/schedule-task/${localTask._id}`);
  ...
  const handleComplete = async () => {
    ...
    try {
      const updatePayload = { completedBy: updatedCompletedBy };
      setLocalTask({ ...localTask, completedBy: updatedCompletedBy as any });
  ...
  const handleFinishTask = async () => {
    ...
    if (authorId === assignedId && user._id === authorId) {
      ...
      updatedCompletedBy = [...filteredCompletedBy, { userId: user._id }];
    }

    try {
      const updatePayload = { status: 'completed', completedBy: updatedCompletedBy };
      setLocalTask({
        ...localTask,
        status: 'completed',
  ...
  const handleReassign = async () => {
    ...
    try {
      ...
      const response = await axiosInstance.patch(`/task/reassign/${localTask._id}`);
  ...
  const handleSaveChanges = async () => {
    ...
    const newDueDate = tempDueDate ? new Date(tempDueDate).toISOString() : null;
    if (newDueDate !== localTask.dueDate) {
    ...
    if (
      ...
      if (scheduledDate !== (localTask.scheduledDate ?? null)) {
        updates.scheduledDate = scheduledDate;
        updates.scheduledDays = null;
        hasChanges = true;
      }
    } else {
      if (localTask.scheduledDate != null) {
  ...
  const handleMarkAsImportant = async () => {
    ...
    setLocalTask({ ...localTask, importantBy: newImportantBy });
    try {
      await onUpdate({ importantBy: newImportantBy });
    } catch (error) {
      console.error('Failed to update importance:', error);
      setIsImportant(!willBeImportant);
      setLocalTask({ ...localTask, importantBy: currentImportantBy });
    }
  };
  ...
  const isDueDateLocked = ['daily', 'weekly', 'monthly'].includes(
    frequency as string
  );

  const getAssignedId = () => {
  if (!localTask?.assigned) return null;
  return typeof localTask.assigned === 'string'
    ? localTask.assigned
    : localTask.assigned._id;
};

const isAuthorAndAssigned =
  user?._id === getAuthorId() &&
  user?._id === getAssignedId();
                ...
                <Dialog open={isEditOpen} onOpenChange={setIsEditOpen}>
                  ...
                  <DialogContent className="max-h-[90vh] w-[85vw] max-w-[1200px] overflow-y-auto">
                    ...
                    <div className="grid gap-4 py-4">
                      ...
                      {frequency === TaskFrequency.WEEKLY && (
                        <div className="space-y-3 rounded-xl border border-gray-100 bg-slate-50 p-4">
                          ...
                          <div className="flex flex-wrap gap-2">
                            {['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'].map(
                              (dayLabel, dayIndex) => {
                                ...
                                const isSelected = scheduledDate === dayIndex;
                                ...
                                return (
                                  <Button
                                    ...
                                    onClick={() => {
                                      setScheduledDate(dayIndex);
                                      setTempDueDate(candidate.toDate());
                                    }}
                                  >
                                    {dayLabel}
                                  </Button>
                          ...
                          {tempDueDate &&
                            ...
                            scheduledDate !== undefined && (
                              <p className="mt-1 text-xs text-muted-foreground">
                                ...
                                <strong>
                                  {moment(tempDueDate).format('dddd')}
                                </strong>
                                . Next due:{' '}
                                {moment(tempDueDate).format('DD MMM YYYY')}
                              </p>
                            )}
                        </div>
                      )}
                            ...
                            {Array.from({ length: 31 }, (_, i) => i + 1).map(
                              (day) => {
                                ...
                                const isSelected =
                                  scheduledDate === day &&
                                  tempDueDate &&
                                  moment(tempDueDate).isSame(candidate, 'day');
                  ...
                  <AlertDialogContent className='bg-white'>
                    <AlertDialogHeader>
                      ...
                      <AlertDialogDescription>
                        ...
                        <strong>"{localTask.taskName}"</strong>? This action
                        cannot be undone.
                      </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
