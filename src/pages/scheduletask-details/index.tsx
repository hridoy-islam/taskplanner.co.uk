...
export default function ScheduleTaskDetailsPage() {
  ...
  const fetchTask = async () => {
    ...
    setLoading(true);
    try {
      const res = await axiosInstance.get(`/schedule-task/${id}`);
  ...
  useEffect(() => {
    const markAsSeen = async () => {
      ...
      if (task && user?._id === assigneeId && task.seen === false) {
        try {
          await axiosInstance.patch(`/schedule-task/${task._id}`, { seen: true });
...
const onUpdate = async (updatedData: Partial<Task>) => {
    ...
    try {
      const res = await axiosInstance.patch(`/schedule-task/${id}`, updatedData);
