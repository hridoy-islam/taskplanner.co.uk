...
export default function GroupChat() {
  ...
  const handleMemberDialog = () => {
    ...
    const loadMembers = async () => {
      ...
      const currentMemberIds = groupDetails?.members?.map((member: any) => member._id) || []
      const nonGroupMembers = fetchedMembers.filter((member: any) => !currentMemberIds.includes(member._id))
      setInitialMembers(
  ...
  const updateLastReadMessage = async (groupId, userId, messageId) => {
    try {
      ...
      const response = await axiosInstance.post(`/group/updatereadmessage`, body)
    } catch (error) {
    ...
    try {
      ...
      setComments((prevComments) => {
        const newComments = fetchedComments.filter(
          (newComment) => !prevComments.some((comment) => comment._id === newComment._id),
        )
  ...
  const calculateScrollPosition = () => {
    if (commentsEndRef.current) {
      return commentsEndRef.current.scrollHeight - commentsEndRef.current.scrollTop
    }
    return 0
  }
  ...
  const applyScrollPosition = (scrollPosition) => {
    if (commentsEndRef.current) {
      commentsEndRef.current.scrollTop = commentsEndRef.current.scrollHeight - scrollPosition
    }
  }
  ...
  const handleGroupDescriptionUpdate = async (e) => {
    ...
    try {
      const response = await axiosInstance.patch(`/group/single/${currentPath}`, data)
      if (response.data.success) {
        toast(`Success!`, {
  ...
  useEffect(() => {
    if (groupDetails?.members) {
      const currentUser = groupDetails?.members.find((member) => member._id === user?._id)
  ...
  return (
    <div className="mx-auto flex h-full max-w-full">
      ...
      <Button
        ...
        ref={buttonRef}
      >
        {isSidebarVisible ? <ArrowLeft className="h-6 w-6" /> : <ArrowRight className="h-6 w-6" />}
      </Button>
