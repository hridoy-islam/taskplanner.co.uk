...
export default function CreateCreator({ onUserCreated }: { onUserCreated: () => void }) {
  ...
  const onSubmit = async (values: FormValues) => {
    ...
    // If the logged-in user is a company, automatically assign the creator to them
    ...
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      ...
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader>
          ...
          <DialogDescription>
            Enter the credentials below to create a new creator account.
          </DialogDescription>
        </DialogHeader>
                ...
                <FormItem className="flex flex-row items-start space-x-3 space-y-0 rounded-md border border-gray-300 p-4 w-full shadow-sm">
                  ...
                  <div className="space-y-1 leading-none">
                    <FormLabel className="cursor-pointer text-sm font-medium">
                      Pre-Authorize Creator
                    </FormLabel>
                    <FormDescription className="text-xs">
                      Immediately grant this creator valid status and access.
                    </FormDescription>
                  </div>
                </FormItem>
              )}
