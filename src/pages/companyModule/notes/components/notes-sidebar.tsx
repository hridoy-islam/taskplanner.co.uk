...
export default function NotesSidebar({
  ...
  return (
    <div className="w-96 p-3 flex flex-col border-r border-taskplanner/60 bg-white">
      ...
      >
            ...
            return (
              <div
                ...
                className={`cursor-pointer rounded-2xl p-5 border border-gray-200 transition-all ${
                  isSelected ? "bg-taskplanner text-white shadow-sm " : "bg-white hover:bg-neutral-50 "
                }`}
                onClick={() => setSelectedNote(note)}
              >
                <div className="flex justify-between items-start mb-2">
                  <h3 className={`text-lg font-bold truncate ${isSelected ? 'text-white' : 'text-black'}`}>{note.title}</h3>
                ...
                <div className="flex flex-wrap gap-2 mb-3">
                  {note.tags?.map((tag: any) => (
                    <span key={tag._id} className={`text-xs font-bold ${isSelected ? 'text-white' : ''}`}>
                      #{tag.name}
                    </span>
                  ))}
                </div>
                <div className={`text-xs ${isSelected ? 'text-white' : 'text-neutral-400'}`}>
        ...
        <TabsContent value="shared-notes" className="flex-1 overflow-y-auto pt-4 space-y-2">
